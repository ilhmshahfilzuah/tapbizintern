...
class _BottomSheet2PageState extends State<BottomSheet2Page> {
              ...
              children: <Widget>[
                    ...
                    GestureDetector(
                      onTap: () {
                        Fluttertoast.showToast(
                            msg: 'Click view all courier',
                            toastLength: Toast.LENGTH_SHORT);
                      },
                      child: Text('view all', style: TextStyle(color: _color2)),
                    )
                  ],
                ),
                Wrap(
                  ...
                  children: List.generate(_courierList.length, (index) {
                    return _filter(_courierSelectedList, _courierList[index],
                        index, mystate);
                  }),
                ),
                SizedBox(
                ...
                Wrap(
                  ...
                  children: List.generate(_conditionList.length, (index) {
                    return _filter(_conditionSelectedList,
                        _conditionList[index], index, mystate);
                  }),
                ),
              ],
            ),
  ...
  Widget _radioSort(String value, int index, mystate) {
    return ChoiceChip(
        label: Text(value,
            style:
                TextStyle(color: _sortIndex == index ? Colors.white : _color1)),
        shape: RoundedRectangleBorder(
            side: BorderSide(
                width: 1,
  ...
  Widget _filter(var selectedData, String value, int index, mystate) {
    return FilterChip(
      ...
      labelStyle: TextStyle(
          color: selectedData[index] == true ? Colors.white : _color1),
      shape: RoundedRectangleBorder(
          side: BorderSide(
              width: 1,
