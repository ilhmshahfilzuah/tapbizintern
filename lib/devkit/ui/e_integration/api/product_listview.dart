...
import 'package:TapBiz/devkit/bloc/product_listview/bloc.dart';
import 'package:TapBiz/devkit/model/integration/product_listview_model.dart';
...
class _ProductListviewPageState extends State<ProductListviewPage> {
  ...
  void initState() {
    ...
    _productListviewProductBloc.add(GetProductListview(
        sessionId: '5f0e6bfbafe255.00218389',
        skip: _apiPage.toString(),
        limit: _apiLimit.toString(),
        apiToken: apiToken));
                  ...
                  if (_lastData && _apiPage == 0) {
                    ...
                    if (productListviewData.length == 0) {
                      ...
                      return ListView.builder(
                        itemCount: (state is ProductListviewWaiting)
                            ? productListviewData.length + 1
                            : productListviewData.length,
                ...
                children: <Widget>[
                  ClipRRect(
                      ...
                      child: buildCacheNetworkImage(
                          width: boxImageSize,
                          height: boxImageSize,
                          url: productListviewData[index].image)),
                  SizedBox(
                    width: 10,
                  ),
                  Expanded(
                    child: Column(
                      ...
                      children: [
                          ...
                          child: Text(
                              '\$ ' +
                                  _globalFunction.removeDecimalZeroFormat(
                                      productListviewData[index].price),
                              style: TextStyle(
                          ...
                          child: Row(
                            children: [
                              _globalWidget.createRatingBar(
                                  rating: productListviewData[index].rating,
                                  size: 12),
                              Text(
                                  '(' +
                                      productListviewData[index]
                                          .review
                                          .toString() +
                                      ')',
                                  style: TextStyle(
                        ...
                        Container(
                          margin: EdgeInsets.only(top: 5),
                          child: Text(
                              productListviewData[index].sale.toString() +
                                  ' Sale',
                              style: TextStyle(
                                  fontSize: 11, color: Color(0xFFaaaaaa))),
                        ),
