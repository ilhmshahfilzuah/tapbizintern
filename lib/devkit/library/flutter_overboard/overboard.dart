...
import 'package:TapBiz/devkit/library/flutter_overboard/circular_clipper.dart';
import 'package:TapBiz/devkit/library/flutter_overboard/overboard_animator.dart';
...
import '../../../subui/auth_b/subsignin.dart';
import '../../../subui/auth_b/subsignup.dart';
...
class OverBoard extends StatefulWidget {
  ...
  OverBoard({Key? key, required this.pages, this.center, this.showBullets, this.skipText, this.nextText, this.finishText, required this.finishCallback, this.animator, this.skipCallback}) : super(key: key);
...
class _OverBoardState extends State<OverBoard> with TickerProviderStateMixin {
                                    ...
                                    for (int i = 0; i < _total; i++)
                                      Padding(
                                        ...
                                        child: AnimatedContainer(
                                            ...
                                            decoration: BoxDecoration(color: (i == _counter) ? AppSettings.ColorMain : Colors.grey[300], borderRadius: BorderRadius.circular(10))),
                                      )
                                  ],
                                ),
                              )
                ...
                Row(
                  children: [
                    TextButton(
                        style: ButtonStyle(
                          foregroundColor: MaterialStateProperty.resolveWith<Color>(
                            (Set<MaterialState> states) => AppSettings.ColorMain,
                          ),
                        ),
                        onPressed: () {
                                Navigator.pushReplacement(context, new MaterialPageRoute(builder: (context) => SubSignupPage()));
                              },
                        child: Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          child: Text("Register", style: TextStyle(fontSize: 20, color: AppSettings.ColorMain, fontWeight: FontWeight.w700),),
                        )),
                    TextButton(
                        style: ButtonStyle(
                          foregroundColor: MaterialStateProperty.resolveWith<Color>(
                        ...
                        child: Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 16),
                          child: Text("Sign In", style: TextStyle(fontSize: 20, color: AppSettings.ColorMain, fontWeight: FontWeight.w700),),
                        )),
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: <Widget>[
                        page.doAnimateImage!
                            ? AnimatedBoard(
                                ...
                                child: new Padding(
                                  ...
                                  child: (page.imageAssetPath != null) ? new Image.asset(page.imageAssetPath!, width: 300.0, height: 300.0) : buildCacheNetworkImage(width: 300, height: 300, url: page.imageFromUrl, plColor: Colors.transparent),
                                ),
                              )
                            : new Padding(
                                padding: new EdgeInsets.only(bottom: 25.0),
                        ...
                        Padding(
                          padding: new EdgeInsets.only(top: 10.0, bottom: 30.0, left: 30.0, right: 30.0),
                          child: new Text(
                            page.title!,
                            textAlign: TextAlign.center,
                            style: new TextStyle(color: Colors.black, fontSize: 24.0, fontWeight: FontWeight.bold),
                          ),
                        ),
                        Padding(
  ...
  _animate() {
    ...
    if (_scroll > _bulletContainerWidth && _swipeDirection == SwipeDirection.RIGHT_TO_LEFT) {
      ...
      _scrollController.animateTo(_scrollDistance, curve: Curves.easeIn, duration: Duration(milliseconds: 100));
    } else if (_scroll < (_maxScroll - _bulletContainerWidth) && _swipeDirection == SwipeDirection.LEFT_TO_RIGHT) {
      _scrollController.animateTo(_scroll, curve: Curves.easeIn, duration: Duration(milliseconds: 100));
    } else if (_swipeDirection == SwipeDirection.SKIP_TO_LAST) {
      _scrollController.animateTo(_maxScroll, curve: Curves.easeIn, duration: Duration(milliseconds: 100));
    }
  }
}
...
class AnimatedBoard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Transform(
      transform: new Matrix4.translationValues(0.0, 50.0 * (1.0 - animator!.getAnimator().value), 0.0),
      child: new Padding(
        padding: new EdgeInsets.only(bottom: 25.0),
        child: child,
      ),
